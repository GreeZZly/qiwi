...
<br>
<br>ПРОЦЕНТ: <font color="#FF860D"><?php echo $stvar; ?>% </font>
<hr>
...
<div class="refs_link" style=" margin-top: 0px; border: 1px solid #dfdfdf; color: #000000; background: none;">Реферальная ссылка: <font color="#5d8fd1">http://<?php echo $site; ?>/?ref=<?php echo $u_login; ?></font></div>
<?php
$refstotq=mysql_query("SELECT login FROM users WHERE ref='".$u_login."'");
$refsq=mysql_query("SELECT ologin,sum(osum2),sum(orefsum) FROM operations WHERE otype=3 AND oref='".$u_login."' AND ologin!='".$u_login."' AND osum2>0 AND obatch!='' AND oback='' GROUP BY ologin ORDER BY sum(osum2) DESC");
$refsar1=array();
...
Рефералов: <font><?php echo mysql_num_rows($refstotq); ?></font>
<br>Активно: <font><?php echo mysql_num_rows($refsq); ?></font>
<?php
...
$prib_text_1.='&nbsp;'.$refsm[0].'&nbsp;<font color="#FF8D1C" style="padding-left: 80px;">'.str_replace('.00','',number_format($refsm[1],2,'.',',')).'</font>&nbsp;<font color="#3EAA30">РУБ</font>, ';
$prib+=$refsm[2];
...
?>
<br>Прибыль: <font><?php echo number_format($prib,2,'.',','); ?> РУБ</font>
</td>
</tr>
</tbody></table>
...
<div class="refs_rb">
Рефбэк - процент от прибыли за вклад с QIWI Вашим приглашённым, который автоматически отправится ему же на баланс как подарок.
<br>Пример: рефбэк 20% прибыль с приглашённого 100 рублей. 20% идут ему, а 80% Вам.
</div>

</div>
<table align="center" class="withdrawal_stat" cellpadding="0px" cellspacing="0px">
<tbody><tr>
<td width="150px" style="padding-left:40px;">Реферал</td>
